...
Rispondi SOLO con il JSON, niente altro testo.` });
      const res = await fetch("https://cs-energia-proxy.roberta-esposito.workers.dev", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 2500, messages: [{ role: "user", content: imgContent }] }),
      });
      const data = await res.json();
      ...
      <div style={{ padding: "28px", maxWidth: "900px", margin: "0 auto" }}>
        ...
        <div style={{ background: CARD2, border: "2px solid " + P, borderRadius: "12px", overflow: "hidden", marginBottom: "20px" }}>
          ...
          {(() => {
            ...
            { label:"ORE",          actual:fv(r.ore),              tobe:(<span title={oreTitle}>{fv(r.oreDich)+oreStar+" dich."}</span>),          delta:calcPct(r.ore,r.oreDich),         rule:"standard" },
              ...
              {filtered.map((r, i) => (
                <div key={r.id} style={{ display: "grid", gridTemplateColumns: mainTpl, gap: "6px", padding: "11px 16px", background: i % 2 === 0 ? CARD : "#150e38", borderBottom: "1px solid " + BD, alignItems: "center", fontSize: "13px", fontFamily: FF, minWidth: "1100px" }}>
                  ...
                  <div style={{ display: "flex", gap: "5px" }}>
                    ...
                    <button onClick={async () => { const upd = enrichWithInheritedTobe(records.filter(x => x.id !== r.id)); setRecords(upd); await persist(upd); }} style={{ background: "#c0392b22", border: "none", color: "#ff5c5c", borderRadius: "5px", padding: "5px 9px", cursor: "pointer", fontFamily: FF, fontSize: "13px" }}>🗑</button>
                  </div>
                </div>
              ))}
              {periodAvg && (
