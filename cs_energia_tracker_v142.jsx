...
function aggregateExcelToRecord(parsedFiles, targetDate) {
  ...
  parsedFiles.forEach(({ type, period, parsed }) => {
    ...
    if (type === "economics" && (period === "giornaliero" || period === "settimanale")) {
      if (parsed.total) {
        record.leadGen = parsed.total.leads || record.leadGen;
        record.cpl = parsed.total.cpl != null ? Number(parsed.total.cpl.toFixed(2)) : record.cpl;
        record.red = parsed.total.red != null ? Number((parsed.total.red * 100).toFixed(1)) : record.red;
        record.cpa = parsed.total.cpaTotNew != null ? Math.round(parsed.total.cpaTotNew) : record.cpa;
      }
      // Macrofornitori (escludo "Total")
      if (parsed.fornitori && parsed.fornitori.length > 0) {
        record.macrofornitori = parsed.fornitori
          .filter(f => f.nome && f.leads != null && f.leads > 0)
          .map(f => ({
            nome: f.nome,
            leads: f.leads,
            pctTot: f.pctLead != null ? Number((f.pctLead * 100).toFixed(2)) : null,
            cpl: f.cpl != null ? Number(f.cpl.toFixed(2)) : null,
            red: f.red != null ? Number((f.red * 100).toFixed(1)) : null,
            cpa: f.cpaTotNew != null ? Math.round(f.cpaTotNew) : null,
          }));
        // % CTC = quota GOOGLE CLICK + Trabocchi (semplificato: GALANO GOOGLE CTC + TRABOCCO TLC + TRABOCCO A&R + RUIU GOOGLE CTC)
        const ctcNomi = ["GALANO GOOGLE CTC", "TRABOCCO TLC", "TRABOCCO A&R", "RUIU GOOGLE CTC"];
        const ctcLeads = record.macrofornitori
          .filter(m => ctcNomi.some(n => m.nome.toUpperCase().includes(n)))
          .reduce((sum, m) => sum + (m.leads || 0), 0);
        if (record.leadGen && ctcLeads > 0) {
          record.ctc = Number(((ctcLeads / record.leadGen) * 100).toFixed(2));
        }
      }
    }

    // ECONOMICS MESE (cumulato 1→giorno) → cpl mese, cpa mese, red mese
    if (type === "economics" && (period === "cumulato_mese" || period === "mese_intero")) {
      if (parsed.total) {
        record.cplMese = parsed.total.cpl != null ? Number(parsed.total.cpl.toFixed(2)) : record.cplMese;
        record.cpaMese = parsed.total.cpaTotNew != null ? Math.round(parsed.total.cpaTotNew) : record.cpaMese;
        record.redMese = parsed.total.red != null ? Number((parsed.total.red * 100).toFixed(1)) : record.redMese;
      }
    }

    // CONTATTAB GIORNALIERO (cluster Selezioni multiple, netto CTC) → contattab, timing, pressione, durata, hourly
    if (type === "contattab" && period === "giornaliero") {
      if (parsed.total) {
        record.contattab = parsed.total.contattab != null ? Number((parsed.total.contattab * 100).toFixed(1)) : record.contattab;
        // Timing netto CTC, ma il timing principale lo prendiamo dal cluster Tutte (file timing)
        record.pressione = parsed.total.pressione != null ? Number(parsed.total.pressione.toFixed(2)) : record.pressione;
        record.durMedia = parsed.total.durata != null ? Number(parsed.total.durata.toFixed(2)) : record.durMedia;
      }
      if (parsed.hourly && parsed.hourly.length > 0) {
        record.hourlySelezMultiple = parsed.hourly.map(h => ({
          ora: h.ora,
          leads: h.leads,
          contattab: h.contattab != null ? Number((h.contattab * 100).toFixed(1)) : null,
          timing: h.timing != null ? Number((h.timing * 100).toFixed(1)) : null,
          red: h.red != null ? Number((h.red * 100).toFixed(1)) : null,
        }));
      }
    }

    // CONTATTAB MESE → contattabMese, (timingMese si prende da timing_mese)
    if (type === "contattab" && (period === "cumulato_mese" || period === "mese_intero")) {
      if (parsed.total) {
        record.contattabMese = parsed.total.contattab != null ? Number((parsed.total.contattab * 100).toFixed(1)) : record.contattabMese;
      }
    }
    ...
    if (type === "timing" && period === "giornaliero") {
      if (parsed.total) {
        record.timing = parsed.total.timing != null ? Number((parsed.total.timing * 100).toFixed(1)) : record.timing;
      }
      if (parsed.hourly && parsed.hourly.length > 0) {
    ...
    // TIMING MESE → timingMese + hourlyMese
    if (type === "timing" && (period === "cumulato_mese" || period === "mese_intero")) {
      if (parsed.total) {
        record.timingMese = parsed.total.timing != null ? Number((parsed.total.timing * 100).toFixed(1)) : record.timingMese;
      }
      if (parsed.hourly && parsed.hourly.length > 0) {
        record.hourlyMese = parsed.hourly.map(h => ({
          ora: h.ora,
          leads: h.leads,
          contattab: h.contattab != null ? Number((h.contattab * 100).toFixed(1)) : null,
          timing: h.timing != null ? Number((h.timing * 100).toFixed(1)) : null,
    ...
    if (type === "resa_net" && parsed.total) {
      // Trovo la colonna del target date (se specificata) altrimenti uso colonna Total
      const dateKey = Object.keys(parsed.total).find(k => k !== "totale");
      if (parsed.total.totale != null) {
        record.resaMese = Number(parsed.total.totale.toFixed(2));
      }
      // Cerco la data più recente come "giornaliera"
      const dataKeys = Object.keys(parsed.total).filter(k => k !== "totale" && parsed.total[k] != null).sort();
      if (dataKeys.length > 0) {
        const lastKey = dataKeys[dataKeys.length - 1];
        record.resa = Number(parsed.total[lastKey].toFixed(2));
      }
    }
    ...
    if (type === "ore" && parsed.total) {
      ...
      if (dataKeys.length > 0) {
        const lastKey = dataKeys[dataKeys.length - 1];
        record.ore = Math.round(parsed.total[lastKey]);
      }
      if (parsed.total.totale != null) {
        record.oreMese = Math.round(parsed.total.totale);
      }
    }
    ...
    if (type === "lavora" && parsed.total) {
      ...
      if (dataKeys.length > 0) {
        const lastKey = dataKeys[dataKeys.length - 1];
        record.lavOra = Number(parsed.total[lastKey].toFixed(2));
      }
      if (parsed.total.totale != null) {
        record.lavOraMese = Number(parsed.total.totale.toFixed(2));
      }
    }
  });
...
function QuickInsert({ onSaved, onExtracted, records }) {
  ...
  const extractFromExcel = () => {
    ...
    const dateInput = manualDate || (() => {
      // Cerco data dai filtri Excel
      const economicsFile = excelFiles.find(f => f.type === "economics" && f.parsed && f.parsed.filtri);
      if (economicsFile && economicsFile.parsed.filtri.startDate) {
        const d = economicsFile.parsed.filtri.startDate;
        return String(d.getDate()).padStart(2, "0") + "/" + String(d.getMonth() + 1).padStart(2, "0");
      }
      return "";
    })();
