...
function SintesiBox({ r, records, onSave }) {
  ...
  const buildCtx = () => {
    ...
    const patternLines = patternActions.length>0
      ? ["PATTERN_MESE (precalcolato — fasce con timing mese fuori range 90-97%. Non classificare cause, solo elencare):"].concat(patternActions)
      : ["PATTERN_MESE: nessuna fascia fuori range nel mese"];
    ...
    ranked.forEach((name, i) => {
      ...
      macroRankLines.push(
        ...
        segnaleGiorno + trendLabel + mediaLabel + bestForLabel +
        " → AZIONE: " + azione
      );
    ...
    const riallocCandidati = ranked
      ...
      .map(n => ({ name: n, media3gg: macroMeseMap[n]._media3gg }))
      .sort((a,b) => a.media3gg - b.media3gg)
      .slice(0, 2);
