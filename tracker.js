...
function WeeklyView({
  ...
  const generaConfronto = async () => {
    ...
    const corrente = {
      ...
      leadGg: avgLeadGg,
      tobeLeadGg: avgTobeLeadGg,
      oreGg: avgOreGg,
      tobeOreGg: avgTobeOreGg,
      arpu: arpuSett,
      kpi: k,
      macrofornitori: macroRows.map(m => ({
        ...
        deltaVsTobe: m.delta != null ? parseFloat(m.delta.toFixed(1)) : null,
    ...
    const prompt = "Sei un analista marketing operativo di ComparaSemplice.it. Confronta due settimane di performance del tracker CS Energia.\n\n" + "STILE OBBLIGATORIO (livello secco):\n" + "- Frasi brevi, essenziali, zero retorica\n" + "- Usa il formato 'valore_prec → valore_corrente (delta)' quando possibile\n" + "- Esempio: 'RED: 16,9% → 15,6% (-1,3pp)'\n" + "- Esempio: 'Lead/gg: 2.389 → 2.385 (-0,2%)'\n" + "- Zero 'basis points', zero 'consolidation', zero 'deterioration'\n\n" + "UNITÀ DI MISURA — tassative:\n" + "- Per grandezze assolute (lead, ore, €): usa variazione percentuale, es. '-5,8%' o '+€4'\n" + "- Per KPI già in % (RED, CTC, timing, contattabilità, resa): usa punti percentuali, es. '-1,3pp'\n" + "- NON usare MAI 'basis points' o 'bp'\n\n" + "STRUTTURA (3 paragrafi, max 120 parole totali):\n" + "- Paragrafo 1 — Volumi e produttività: lead/gg, ore/gg, LAV/ORA con delta\n" + "- Paragrafo 2 — Economics: CPA, CPL, RED con delta. Identifica chi guida il peggioramento/miglioramento\n" + "- Paragrafo 3 — Macrofornitori: 2-3 più rilevanti per volume/variazione. Se c'è una nota positiva, aggiungila alla fine\n\n" + "REGOLE:\n" + "- Solo prosa, niente bullet, niente emoji, niente markdown (no **grassetto**, no ### header)\n" + "- Paragrafi separati da doppio a capo\n" + "- Se un valore è null/mancante nei dati, ometti quella metrica (non inventare)\n" + "- Italiano corretto e scorrevole\n\n" + "SETTIMANA CORRENTE: " + JSON.stringify(corrente) + "\n\n" + "SETTIMANA PRECEDENTE: " + JSON.stringify(precedente);
    try {
      const res = await fetch("https://cs-energia-proxy.roberta-esposito.workers.dev", {
        method: "POST",
        headers: {
  ...
  }, "\u2014 Confronto vs settimana ", fmtDDMM(settPrec.mon), "-", fmtDDMM(settPrec.sat), " \u2014"), /*#__PURE__*/React.createElement("button", {
  ...
  }, "Errore generazione confronto: ", errConfronto), record.confrontoSettPrec?.testo ? /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "14px 16px",
      background: "rgba(102,76,205,0.05)",
  ...
  }, "Confronto non ancora generato. Premi \"\u21BB Rigenera\" per crearlo.")));
}
function App() {
