...
function WeeklyLoadForm({
  ...
  const handleExtract = async () => {
    ...
    const textPrompt = "Estrai i dati dalla tabella Power BI per un range SETTIMANALE (6 giorni lun-sab). Restituisci SOLO un JSON valido senza testo aggiuntivo.\n\n" + "Struttura richiesta:\n" + "{\n" + '  "kpi": {\n' + '    "red": numero (es. 19.4),\n' + '    "resa": numero (es. 0.48),\n' + '    "ctc": numero (es. 25.8),\n' + '    "trabocco": numero (% Trabocco TLC sul totale CTC, es. 0.75),\n' + '    "timing": numero (timing 1-5min, es. 89),\n' + '    "contattab": numero (contattabilità netto CTC, es. 61.2),\n' + '    "cpl": numero,\n' + '    "cpa": numero,\n' + '    "lavOra": numero\n' + "  },\n" + '  "hourlySett": { "9": 74, "10": 81, ..., "19": 88 } (timing medio settimana per fascia),\n' + '  "macrofonti": [\n' + '    { "name": "GALANO GOOGLE CTC", "leads": numero (lead/gg medio settimana), "pct": "22.5", "cpl": "12.3", "cpa": "88", "red": "23.1" },\n' + '    ... (tutti i fornitori inclusi TRABOCCO TLC, TRABOCCO A&R, ETC)\n' + '  ]\n' + "}\n\n" + "Regole:\n" + "- LEAD GENERATE e ORE vengono calcolati separatamente dai record giornalieri, NON estrarli.\n" + "- Tutti gli altri KPI vanno dalla sezione Dashboard Economics (cluster Tutte) per il range settimanale.\n" + "- Contattab (netto CTC) va dalla tabella cluster Selezioni multiple.\n" + "- Se un valore non è presente o è '—', usa null.\n" + "- Nessuna classificazione, solo numeri grezzi.";
    try {
      const res = await fetch("https://cs-energia-proxy.roberta-esposito.workers.dev", {
        method: "POST",
        headers: {
...
function WeeklyView({
  ...
  // Calcolo Lead/gg e Ore/gg come media dai record giornalieri del range
  ...
  for (let i = 0; i < 6; i++) {
    ...
    dayKeys.push(fmtDDMM(d));
  }
  const daysInRange = records.filter(r => dayKeys.includes(r.date));
  const avgLeadGg = daysInRange.length > 0 ? Math.round(daysInRange.reduce((s, r) => s + (parseInt(r.leadGen) || 0), 0) / daysInRange.length) : null;
  const avgOreGg = daysInRange.length > 0 ? Math.round(daysInRange.reduce((s, r) => s + (parseFloat(r.ore) || 0), 0) / daysInRange.length) : null;

  // TO BE settimanale: somma TO BE lun-ven + sabato
  ...
  const tobeLunVenLead = daysInRange.slice(0, 5).reduce((s, r) => s + (parseInt(r.leadToBe) || 0), 0);
  const tobeLunVenOre = daysInRange.slice(0, 5).reduce((s, r) => s + (parseFloat(r.oreDich) || 0), 0);
  const totalTobeLead = sabato ? tobeLunVenLead + sabato.lead : tobeLunVenLead;
  const totalTobeOre = sabato ? tobeLunVenOre + sabato.ore : tobeLunVenOre;
  const avgTobeLeadGg = totalTobeLead > 0 ? Math.round(totalTobeLead / 6) : null;
  const avgTobeOreGg = totalTobeOre > 0 ? Math.round(totalTobeOre / 6) : null;
  const k = record.kpi || {};
  ...
  }, "KPI Settimana ", fmtDDMM(activeWeek.mon), "-", fmtDDMM(activeWeek.sat), " (6 giorni lun-sab)"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#8a7ca8",
      marginBottom: "12px"
    }
  }, "Lead/Ore da media giornaliera \xB7 Altri KPI da screenshot settimanale \xB7 ARPU: \u20AC", record.arpu), /*#__PURE__*/React.createElement("table", {
    style: {
      width: "100%",
  ...
  }, avgTobeOreGg !== null ? avgTobeOreGg + " dich." : "—")), /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
    style: {
      padding: "10px 8px",
      color: O,
      fontWeight: 500,
