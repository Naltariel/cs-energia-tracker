...
function SintesiBox({
  ...
  const buildCtx = () => {
    ...
    ranked.forEach((name, i) => {
      ...
      // Cappato = volume sotto TO BE ≥ 10% E CPA mese sopra target (scelta marketing di tagliare volume su fornitore costoso)
      const cappato = deltaVsTobe !== null && parseFloat(deltaVsTobe) <= -10 && cpaMeseNum > CPA_TARGET;
      ...
      // ── LOGICA AZIONE ──
      // Schema:
      // 1. CAPPATO (CPA mese >100 + volume ≤-10%) → azione dipende dal trend
      // 2. NON CAPPATO + CPA critico + volume OK → RIDURRE proporzionalmente
      // 3. NON CAPPATO + CPA ottimale + sotto TO BE → AUMENTARE
      // 4. Tutto OK → MANTENERE (salta paragrafo)
      const TARGET_CPA = 100;
      const deltaVolNum = deltaVsTobe !== null ? parseFloat(deltaVsTobe) : null;
      let azione;
      if (cappato) {
        // Fornitore cappato dal marketing: azione dipende dal trend CPA
        if (trendDir === "IN_SALITA") {
          azione = "AUMENTARE_CAP [trend_CPA_in_peggioramento]";
        } else if (trendDir === "IN_CALO") {
          azione = "MANTENERE_CAP [trend_CPA_in_miglioramento]";
        } else {
          // STABILE o ALTALENANTE
          azione = "MANTENERE_CAP [trend_stabile_o_altalenante]";
        }
      } else if (cpaMeseNum > TARGET_CPA) {
        // Non cappato con CPA critico: riduzione proporzionale
        let leadsRiduzione = 0;
        if (cpaMeseNum > 140) leadsRiduzione = 40;else if (cpaMeseNum > 120) leadsRiduzione = 30;else if (cpaMeseNum > 110) leadsRiduzione = 18;else leadsRiduzione = 10;
        azione = "RIDURRE_" + leadsRiduzione + "_LEAD/GG";
      } else if (cpaMeseNum <= TARGET_CPA && deltaVolNum !== null && deltaVolNum < -5) {
        // CPA ottimale ma fornitore under-genera: aumentare
        azione = "AUMENTARE_fino_a_40_lead/gg_se_RED_sopra_soglia";
      } else {
        // Tutto ok: nessuna azione necessaria
        azione = "MANTENERE";
      }
...
REGOLA FONDAMENTALE:
- Scrivi un paragrafo SOLO per i fornitori principali (NON TRABOCCO, NON ETC) il cui tag AZIONE nei DATI inizia con: RIDURRE, AUMENTARE, AUMENTARE_CAP, MANTENERE_CAP.
...
TAG=AUMENTARE_fino_a_40_lead/gg_se_RED_sopra_soglia:
...
TAG=AUMENTARE_CAP [trend_CPA_in_peggioramento]:
"[NOME]: ieri ha registrato [DICHIARAZIONE_CPA]. Trend CPA ultimi 3 giorni: in peggioramento, media €[MEDIA_CPA_3GG]. Volume già ridotto [delta_vol]% vs TO BE (fornitore cappato). Azione: Aumentare il cap, il trend CPA è in peggioramento."

TAG=MANTENERE_CAP [trend_CPA_in_miglioramento]:
"[NOME]: ieri ha registrato [DICHIARAZIONE_CPA]. Trend CPA ultimi 3 giorni: in miglioramento, media €[MEDIA_CPA_3GG]. Volume ridotto [delta_vol]% vs TO BE (fornitore cappato). Azione: Cap efficace, mantenere il cap e monitorare."

TAG=MANTENERE_CAP [trend_stabile_o_altalenante]:
"[NOME]: ieri ha registrato [DICHIARAZIONE_CPA]. Trend CPA ultimi 3 giorni: [DIREZIONE_LEGGIBILE], media €[MEDIA_CPA_3GG]. Volume ridotto [delta_vol]% vs TO BE (fornitore cappato). Azione: Mantenere il cap attuale."
...
MEDIA_CPA_3GG: leggi il valore MEDIA_CPA_3GG=€X dalla riga fornitore nei DATI. Se non presente, ometti la frase "Trend CPA ultimi 3 giorni...".

delta_vol: leggi dal campo "delta=[X]%_vs_TO_BE" nella riga fornitore nei DATI GIORNO. Es: delta=-35.2%_vs_TO_BE → scrivi "-35,2%".

TOP1_RIALLOC e TOP2_RIALLOC: leggi i nomi da RIALLOCAZIONE_TOP_2 nei dati. Solo per paragrafi con TAG=RIDURRE.

REGOLE AGGIUNTIVE:
- NON inventare motivi non presenti nei tag. Usa SOLO le formulazioni fornite sopra.
- NON scrivere "media sotto target" o simili — se la frase non è nelle regole sopra, NON scriverla.
