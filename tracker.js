...
function SintesiBox({
  ...
  const buildCtx = () => {
    ...
    // RANKING RIALLOCAZIONE: top 2 fornitori con miglior media CPA 3gg (esclusi TRABOCCO ed ETC)
    const riallocCandidati = ranked.filter(n => macroMeseMap[n]._media3gg != null && macroMeseMap[n]._media3gg > 0).map(n => ({
      name: n,
      media3gg: macroMeseMap[n]._media3gg
    })).sort((a, b) => a.media3gg - b.media3gg).slice(0, 2);
    const riallocLine = riallocCandidati.length > 0 ? "\nRIALLOCAZIONE_TOP_2 (migliori media CPA 3gg per riallocare leads): " + riallocCandidati.map(x => x.name + " (media 3gg €" + x.media3gg + ")").join(", ") : "\nRIALLOCAZIONE_TOP_2: nessun fornitore con media CPA 3gg disponibile";
...
Formato: [NOME]: [delta_vs_TO_BE] · RED ieri [X]% [alert_RED] · CPA ieri €X [alert_CPA]
...
Per ogni fornitore principale (NON TRABOCCO, NON ETC) per cui l'AZIONE precalcolata NON è "MANTENERE" e NON è "MANTENERE [CPA_ieri_in_zona_accettabile]", scrivi un paragrafo con questa struttura:

[NOME]: ieri ha registrato [DICHIARAZIONE_CPA]. Trend CPA ultimi 3 giorni: [DIREZIONE_LEGGIBILE], media €[MEDIA_CPA_3GG]. Azione: [AZIONE_DESCRITTIVA] di circa [N] lead/gg. Riallocare su [TOP1_RIALLOC] e [TOP2_RIALLOC] (migliori media CPA 3gg).
...
MEDIA_CPA_3GG: leggi il valore MEDIA_CPA_3GG dalla riga fornitore nei DATI.

AZIONE_DESCRITTIVA + N:
- AZIONE=RIDURRE_X → "Ridurre" (N = X)
- AZIONE=AUMENTARE → "Portare al TO BE (aumentare)" (NON indicare numero lead, scrivi solo "fino a +40 lead/gg se RED sopra soglia")
- AZIONE=MONITORARE → "Monitorare senza ridurre" (niente N)
- AZIONE=MANTENERE_CAP → "Mantenere il cap attuale" (niente N)

TOP1_RIALLOC e TOP2_RIALLOC: leggi i nomi da RIALLOCAZIONE_TOP_2 nei dati. Se c'è un solo nome, scrivi solo quello. Se non c'è nessun nome, ometti la frase "Riallocare su...".

ESEMPIO CORRETTO:
"GALANO SOCIAL: ieri ha registrato il CPA migliore del mese (CPA giorno €122 vs CPA mese €130). Trend CPA ultimi 3 giorni: stabile, media €125. Azione: Ridurre di circa 30 lead/gg. Riallocare su GALANO GOOGLE CALDE (media 3gg €78) e GALANO GOOGLE CTC (media 3gg €95) (migliori media CPA 3gg)."

REGOLE AGGIUNTIVE:
- Se AZIONE è "MANTENERE [CPA_ieri_in_zona_accettabile]" o "MANTENERE": NON scrivere il paragrafo (salta il fornitore)
- NON scrivere mai azioni per TRABOCCO_ o ETC
- Se ci sono fasce in PATTERN_MESE, dopo le azioni sui fornitori aggiungi una riga: "Pattern mese: fasce [elenco] con timing medio sotto 90% da monitorare per pattern strutturale."
