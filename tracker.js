...
function WeeklyRecapBtn({
  ...
  const generate = () => {
    ...
    if (avgLeadGg != null) L.push("- Lead/gg: " + avgLeadGg.toLocaleString() + (avgTobeLeadGg != null ? " vs " + avgTobeLeadGg.toLocaleString() + " TO BE (" + fmtPct(pct(avgLeadGg, avgTobeLeadGg)) + ")" : ""));
    if (avgOreGg != null) L.push("- Ore/gg: " + avgOreGg.toLocaleString() + (avgTobeOreGg != null ? " vs " + avgTobeOreGg.toLocaleString() + " TO BE (" + fmtPct(pct(avgOreGg, avgTobeOreGg)) + ")" : ""));
    ...
    if (sabato || satLeadEff != null) {
      ...
      if (satLeadEff != null && sabato?.lead) L.push("- Lead: " + satLeadEff.toLocaleString() + " vs " + sabato.lead.toLocaleString() + " TO BE (" + fmtPct(pct(satLeadEff, sabato.lead)) + ")");else if (satLeadEff != null) L.push("- Lead: " + satLeadEff.toLocaleString());
      if (satOreEff != null && sabato?.ore) L.push("- Ore: " + satOreEff.toLocaleString() + " vs " + sabato.ore.toLocaleString() + " TO BE (" + fmtPct(pct(satOreEff, sabato.ore)) + ")");else if (satOreEff != null) L.push("- Ore: " + satOreEff.toLocaleString());
      if (satLeadEff != null && satOreEff != null && satOreEff > 0) L.push("- LAV/ORA: " + (satLeadEff / satOreEff).toFixed(2).replace(".", ","));
      L.push("");
    }

    // Timing fasce fuori range
    if (fasceAnalisi && fasceAnalisi.length > 0) {
      L.push("Timing fasce fuori range");
      if (sintesiFasce && sintesiFasce.length > 0) L.push(sintesiFasce.join("; ") + ".");
      L.push("");
    }

    // Pattern mese
    if (patternMeseOver && patternMeseOver.length > 0 || patternMeseUnder && patternMeseUnder.length > 0) {
    ...
    if (macroRows && macroRows.length > 0) {
      ...
      macroRows.forEach(m => {
        ...
        L.push(m.azione.sym + " " + m.name + marg + " — " + m.azione.label);
      });
      L.push("");
    }

    // Macrofornitori sintesi (solo i principali con volume)
    if (macroRows && macroRows.length > 0) {
      L.push("Macrofornitori (media lun-ven)");
      macroRows.filter(m => m.avgLead != null && m.avgLead > 0).forEach(m => {
...
function WeeklyView({
  ...
  }, /*#__PURE__*/React.createElement(WeeklyRecapBtn, {
    record: record,
    activeWeek: activeWeek,
    tobeSabati: tobeSabati,
    records: records,
    macroRows: macroRows,
    fasceAnalisi: fasceAnalisi,
    sintesiFasce: sintesiFasce,
    patternMeseOver: patternMeseOver,
    patternMeseUnder: patternMeseUnder,
    latestRecWithMese: latestRecWithMese,
    avgLeadGg: avgLeadGg,
    avgTobeLeadGg: avgTobeLeadGg,
    avgOreGg: avgOreGg,
    avgTobeOreGg: avgTobeOreGg,
    satLeadEff: satLeadEff,
    satOreEff: satOreEff,
    sabato: sabato,
    fmtDDMM: fmtDDMM
  })), /*#__PURE__*/React.createElement("div", {
...
Rispondi in italiano, in modo diretto e operativo. Se mancano dati, indica esattamente quali e da quale dashboard recuperarli.`;
  ...
  const renderHistory = () => {
    ...
    }, "\u258C Report Settimanale"), /*#__PURE__*/React.createElement("div", {
      style: cardCss
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        ...secCss,
