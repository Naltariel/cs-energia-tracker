...
// Cerca prima nel weekId esatto, poi nella settimana immediatamente precedente, poi nella più recente con ARPU.
function findArpuForDate(dateStr, recordsSett) {
  if (!recordsSett || !Array.isArray(recordsSett) || recordsSett.length === 0) return null;
  ...
  if (targetWeekId) {
    ...
    if (exactMatch) return parseFloat(String(exactMatch.arpu).replace(",", "."));
  }
  // Fallback: settimana più recente con ARPU
  const withArpu = recordsSett.filter(r => r.arpu != null && r.arpu !== "");
  if (withArpu.length === 0) return null;
  withArpu.sort((a, b) => (b.id || "").localeCompare(a.id || ""));
  return parseFloat(String(withArpu[0].arpu).replace(",", "."));
}
...
function SintesiBox({ r, records, recordsSett, onSave }) {
  ...
  const buildCtx = () => {
    ...
    const arpuGiorno = findArpuForDate(r.date, recordsSett || []);
    ...
    // Info ARPU per il prompt (segnalo se manca)
    const arpuInfo = arpuGiorno != null
      ? "\nARPU_GIORNO=€" + arpuGiorno.toFixed(2).replace(".", ",") + " (dalla settimana che contiene il " + r.date + ")"
    ...
    return [...kpi, "", ...fasceLines, "", "FASCE_FUORI_RANGE_ELENCO:", ...hourlyLines, "", ...sintesiFasceBlocco, "", ...patternLines, "", ...macroRankLines, "", ...azioniLines, fascia9Info + riallocLine].join("\n");
  };
...
Sottotitolo (riga vuota, poi): "Margine/lead = (ARPU − CPA) × RED · ARPU di riferimento: €[ARPU_GIORNO]"
- Sostituisci [ARPU_GIORNO] con il valore ARPU_GIORNO dai DATI (es. €129,00).
- Se nei DATI c'è "⚠ ARPU_NON_DISPONIBILE", scrivi invece: "Margine/lead non calcolabile: nessun record settimanale con ARPU disponibile per il giorno."
        ...
        {r.macrofonti && r.macrofonti.length > 0 && (() => {
          ...
          <div style={{ background: CARD2, border: "2px solid " + O, borderRadius: "12px", overflow: "hidden", marginTop: "14px" }}>
            <div style={{ background: "linear-gradient(135deg,#8B3A0A,#c45510)", padding: "10px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ color: "#FFD0B5", fontSize: "9px", fontFamily: FF }}>{r.date}{macroToBe ? " · TO BE da 27/03" : " · TO BE n.d."}</span>
            </div>
        ...
        {(() => {
          ...
          return (
            <div style={{ background:CARD2, border:"2px solid #664CCD44", borderRadius:"12px", overflow:"hidden", marginTop:"20px", marginBottom:"20px" }}>
              ...
              <div style={{ background:"linear-gradient(135deg,#2a1f5e,#1a1540)", padding:"14px 20px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <span style={{ color:"#c4b8ff", fontWeight:"bold", fontSize:"12px", fontFamily:FF }}>📞 KPI per Fascia Oraria</span>
                <span style={{ color:MU, fontSize:"9px", fontFamily:FF }}>{r.date} · {hRows.length} fasce{hourlyToBe ? " · TO BE da 27/03" : ""} · click sui valori per modificare</span>
              </div>
