...
function SintesiBox({ r, records, onSave }) {
  ...
  const buildCtx = () => {
    ...
    // ── Sintesi precostruita (riga finale): bollini + raggruppamento per motivo ──
    // Le fasce con stesso motivo vengono raggruppate su una sola riga con bollino all'inizio:
    //   🔴 = NON_ALIMENTATA (timing ≥ 98%, operatori liberi)
    //   🟠 = OVER_GENERAZIONE o CAUSA_SALA (timing < 90%, problemi di throughput/coda)
    // L'AI deve copiare ESATTAMENTE queste righe nella sezione "— TIMING 1-5 MIN — FASCE FUORI RANGE —"
    ...
    const sintesiFasceBlocco = sintesiFasceRighe.length > 0
      ? ["SINTESI_FASCE_PRECOSTRUITA (copia esattamente queste righe, NON modificarle, NON rimuovere i bollini 🔴/🟠, NON unire i gruppi):"].concat(sintesiFasceRighe)
    ...
    return [...kpi, "", ...fasceLines, "", ...sintesiFasceBlocco, "", "FASCE_FUORI_RANGE_ELENCO:", ...hourlyLines, "", ...patternLines, "", ...macroRankLines, fascia9Info + riallocLine].join("\n");
  };
...
Sintesi: SCRIVI ESATTAMENTE le righe di SINTESI_FASCE_PRECOSTRUITA dai DATI, una per riga, INCLUSI i bollini 🔴 e 🟠 e le virgole tra le fasce.
- NON unire fasce di gruppi diversi
- NON rimuovere i bollini
- NON modificare il testo dopo la freccia
- NON aggiungere virgole o punti e virgola tra le righe (vanno su righe separate)
...
Mostra i fornitori in QUESTO ORDINE FISSO: GALANO GOOGLE CTC, GALANO GOOGLE CALDE, SPINUP SOCIAL, GALANO SOCIAL, PAOLO SOCIAL (o RUIU SOCIAL, è lo stesso fornitore rinominato da maggio 2026 — usa il nome trovato nei dati), TRABOCCO TLC, TRABOCCO A&R.
...
REGOLA FONDAMENTALE:
- Scrivi un paragrafo per TUTTI i fornitori principali (GALANO GOOGLE CALDE, GALANO GOOGLE CTC, SPINUP SOCIAL, GALANO SOCIAL, PAOLO SOCIAL o RUIU SOCIAL — è lo stesso fornitore rinominato a maggio 2026, usa il nome trovato nei dati) — MAI saltarne uno.
- Usa il tag AZIONE dai DATI per scegliere il testo giusto.
- Se il tag AZIONE è "MANTENERE" (senza altro), scrivi comunque un paragrafo con formato: "[NOME]: ieri ha registrato [DICHIARAZIONE_CPA]. Trend CPA ultimi 3 giorni: [DIREZIONE_LEGGIBILE], media €[MEDIA_CPA_3GG]. Azione: mantenere la generazione attuale, monitorare."
