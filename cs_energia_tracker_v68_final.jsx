...
function SintesiBox({ r, onSave }) {
  ...
  const buildCtx = () => {
    ...
    const kpi = [
      ...
      "ORE: " + fv(r.ore) + " vs " + fv(r.oreDich) + " dich. (delta " + sign(pct(r.ore,r.oreDich)) + ")",
    ...
    const patternLines = patternActions.length>0
      ? ["PATTERN_MESE (precalcolato, usa questi senza rielaborare):"].concat(patternActions)
      : ["PATTERN_MESE: nessuna fascia strutturalmente fuori range con intervento ≥10%"];
    ...
    const ranked = fornitori5
      ...
      .sort((a,b) => parseFloat(macroMeseMap[b].cpa)-parseFloat(macroMeseMap[a].cpa));
    ...
    ranked.forEach((name, i) => {
      ...
      if(trend3Cpa.length >= 2) {
        ...
        if(trend3Cpa.length === 2) {
          if(varPct < -5) trendDir = "IN_CALO";
          else if(varPct > 5) trendDir = "IN_SALITA";
          else trendDir = "STABILE";
        } else {
          const c0 = trend3Cpa[0].cpa, c1 = trend3Cpa[1].cpa, c2 = trend3Cpa[2].cpa;
          const diff01 = (c1-c0)/c0, diff12 = (c2-c1)/c1;
          if(c2 < c1 && c1 < c0) trendDir = "IN_CALO";
          else if(c2 > c1 && c1 > c0) trendDir = "IN_SALITA";
          else if(Math.abs(diff01) <= 0.05 && Math.abs(diff12) <= 0.05) trendDir = "STABILE";
          else trendDir = "ALTALENANTE";
        }
      }
      const trendLabel = " TREND_3GG=" + trendDir;

      // Azione base da CPA mese
      let azione, leadsRiduzione=0;
      if(cpaMeseNum>CPA_TARGET) {
        if(cpaMeseNum>140) leadsRiduzione=cappato?20:40;
        else if(cpaMeseNum>120) leadsRiduzione=cappato?15:30;
        else if(cpaMeseNum>110) leadsRiduzione=cappato?10:18;
        else leadsRiduzione=0;
      ...
      if(ZONA_ACCETTABILE && azione.startsWith("RIDURRE")) {
        ...
        azione = cappato
          ? "MANTENERE_CAP [CPA_ieri_in_zona_accettabile]"
          : "MANTENERE [CPA_ieri_in_zona_accettabile]";
      } else if(trendDir === "IN_CALO" && azione.startsWith("RIDURRE")) {
        if(varPct < -15) {
          azione = cappato
            ? "MONITORARE [trend_in_discesa_significativa-non_aggiungere_cap]"
            : "MONITORARE [trend_in_discesa_significativa]";
        } else if(varPct >= -15 && varPct < -5) {
          const ridRidotta = Math.round(leadsRiduzione / 2);
          azione = ridRidotta > 0
            ? "RIDURRE_" + ridRidotta + "_LEAD/GG [trend_in_discesa_moderata-riduzione_dimezzata]" + (cappato?" [già cappato]":"")
            : "MONITORARE [trend_in_discesa_moderata]";
        }
      ...
      macroRankLines.push(
        ...
        (g ? " | GIORNO: leads=" + g.leads + (deltaVsTobe?" delta="+(parseFloat(deltaVsTobe)>=0?"+":"")+deltaVsTobe+"%_vs_TO_BE":"") + " CPA=€" + g.cpa + " RED=" + g.red + "%[" + redAlert(name,g.red) + "]" : "") +
        segnaleGiorno + trendLabel +
        " → AZIONE: " + azione
      );
    });
    ...
    traboccoNames.forEach(tName => {
      ...
      macroRankLines.push(
        ...
        (tGiorno ? " | GIORNO: leads=" + tGiorno.leads + (deltaT?" delta="+(parseFloat(deltaT)>=0?"+":"")+deltaT+"%_vs_TO_BE":"") + " CPA=€" + tGiorno.cpa + " RED=" + tGiorno.red + "%[" + redAlert(tName,tGiorno.red) + "]" : "") +
...
Rispondi sempre in italiano, in modo conciso e operativo.`;
  ...
  const extract = async () => {
    ...
    try {
      ...
      imgContent.push({ type: "text", text: `Sei un assistente che estrae dati KPI da screenshot di dashboard Power BI per ComparaSemplice.it (CS Energia).
...
Rispondi SOLO con il JSON.` });
      const res = await fetch("https://cs-energia-proxy.roberta-esposito.workers.dev", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1500, messages: [{ role: "user", content: imgContent }] }),
      });
      const data = await res.json();
  ...
  const confirm = async () => {
    ...
    setConfirming(true);
    try {
      const rec = { ...extracted, id: extracted.date + "_" + Date.now(), hourly: [] };
...
Rispondi SOLO con il JSON.` });
      const res = await fetch("https://cs-energia-proxy.roberta-esposito.workers.dev", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 500, messages: [{ role: "user", content: imgContent }] }),
      });
      const data = await res.json();
      const raw = data.content?.[0]?.text || "";
      const parsed = JSON.parse(raw.replace(/\`\`\`json|\`\`\`/g, "").trim());
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const sheetRecords = await sheetGetRecords();
      const final = sheetRecords && sheetRecords.length > 0
        ? mergeWithSeed(sheetRecords)
        : mergeWithSeed([]);
      setRecords(final);
  ...
  const handleSubmit = async () => {
    const rec = { ...form, id: form.date + "_" + Date.now(), hourly: [] };
  ...
  const extractFromScreens = async () => {
    ...
    try {
      ...
      const res = await fetch("https://cs-energia-proxy.roberta-esposito.workers.dev", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, messages: [{ role: "user", content: imgContent }] }),
      });
      const data = await res.json();
      const raw = data.content?.[0]?.text || "";
    ...
    <div style={{ background: "linear-gradient(135deg,#1a1540,#0d0b1e)", borderBottom: "3px solid " + P, padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
      ...
      <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
        <div style={{ display: "flex", gap: "4px", padding: "6px", background: "#0d0b1e", borderRadius: "10px" }}>
          ...
          <NavBtn label={"Report Settimana (" + records.length + ")"} active={view === "history"} onClick={() => setView("history")} />
  ...
  const renderForm = () => (
    <div onPaste={(e) => { handleFormPaste(e); }} style={{ padding: "28px", maxWidth: "900px", margin: "0 auto" }}>
      ...
      <QuickInsert onSaved={(updated) => { setRecords(updated); setActiveReport(updated[0]); }} />
      {/* TO BE UPLOAD */}
...
Rispondi in italiano, in modo diretto e operativo. Se mancano dati, indica esattamente quali e da quale dashboard recuperarli.`;
      ...
      <div style={{ padding: "28px", maxWidth: "900px", margin: "0 auto" }}>
        ...
        <div style={{ background: CARD2, border: "2px solid " + P, borderRadius: "12px", overflow: "hidden", marginBottom: "20px" }}>
          ...
          {[
            ...
            { label:"LEAD GENERATE",actual:fv(r.leadGen),          tobe:fv(r.leadToBe)+" TO BE",        delta:calcPct(r.leadGen,r.leadToBe),    rule:"standard" },
            { label:"ORE",          actual:fv(r.ore),              tobe:fv(r.oreDich)+" dich.",          delta:calcPct(r.ore,r.oreDich),         rule:"standard" },
        ...
        <SintesiBox r={r} onSave={async (sintesi) => {
  ...
  const renderHistory = () => {
    ...
    return (
      <div style={{ padding: "28px", maxWidth: "1100px", margin: "0 auto" }}>
        <div style={{ ...secCss, color: P }}>▌ Report Settimana — {filtered.length}/{records.length} giorni</div>

        <div style={cardCss}>
          <div style={{ ...secCss, color: O }}>Seleziona settimana</div>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {(() => {
              const weeks = [];
              const now = new Date();
              ...
              for (let w = 0; w < 5; w++) {
                ...
                weeks.push(
                  <button key={w} onClick={() => { setRangeFrom(fmt(wMon)); setRangeTo(fmt(wFri)); }}
                    style={{ background: isActive ? P : "transparent", color: isActive ? "#fff" : P, border: "2px solid " + P, borderRadius: "7px", padding: "9px 16px", fontSize: "13px", fontFamily: FF, cursor: "pointer", fontWeight: "bold", whiteSpace: "nowrap" }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ ...cardCss, textAlign: "center", padding: "50px", color: MU, fontSize: "16px" }}>Nessun dato nel periodo selezionato.</div>
        ) : (
          <div>
            {/* KPI TABLE — usa valori *Mese dal record più recente del periodo */}
            {(() => {
              ...
              const targets = (() => { const mm = lat && lat.date ? lat.date.split("/")[1] : "04"; return MONTHLY_TARGETS[mm] || MONTHLY_TARGETS["04"]; })();

              if (!hasMese) return (
                <div style={{ ...cardCss, borderColor: "#f5a623", borderWidth:"2px", marginBottom:"20px" }}>
                  <div style={{ color:"#f5a623", fontWeight:"bold", fontSize:"13px", fontFamily:FF, marginBottom:"8px" }}>⚠️ Dati aggregati *Mese mancanti per questo periodo</div>
                  <div style={{ color:MU, fontSize:"12px", fontFamily:FF }}>Inserisci il record più recente del periodo con i valori *Mese da Power BI per visualizzare i KPI aggregati.</div>
                </div>
              );

              const fv2 = (v,suf="") => v ? v+suf : "—";
              const kpiRowsSett = [
                { label:"LEAD / GG", actual: lat.leadGen||"—", tobe: (lat.leadToBe||"—")+" TO BE", delta: calcPct(lat.leadGen, lat.leadToBe), rule:"standard" },
                { label:"ORE / GG", actual: lat.ore||"—", tobe: (lat.oreDich||"—")+" dich.", delta: calcPct(lat.ore, lat.oreDich), rule:"standard" },
                { label:"LAV/ORA", actual: fv2(lat.lavOraMese), tobe: (targets.lavOra||2.53)+" target", delta: lat.lavOraMese ? calcPct(lat.lavOraMese, targets.lavOra||2.53) : null, rule:"standard" },
                { label:"RED", actual: fv2(lat.redMese,"%"), tobe: "17,0% target", delta: lat.redMese ? calcPct(lat.redMese, 17.0) : null, rule:"standard" },
                { label:"RESA", actual: fv2(lat.resaMese), tobe: (targets.resa||0.45)+" target", delta: lat.resaMese ? calcPct(lat.resaMese, targets.resa||0.45) : null, rule:"standard" },
                { label:"% CTC", actual: fv2(lat.ctcMese,"%"), tobe: (targets.ctcMax>=100 ? "≥"+targets.ctcMin+"%" : (targets.ctcMin||25)+"-"+(targets.ctcMax||100)+"%")+" target", delta: null, rule:"ctc", ctcVal: toNum(lat.ctcMese), ctcMin: targets.ctcMin||25, ctcMax: targets.ctcMax||100 },
                { label:"TIMING 1-5min", actual: fv2(lat.timingMese,"%"), tobe: "90-97,99% range", delta: null, rule:"timing", timingVal: toNum(lat.timingMese) },
                { label:"CONTATTAB. NETTO CTC", actual: fv2(lat.contattabMese,"%"), tobe: (targets.contattab||60)+"% target", delta: lat.contattabMese ? calcPct(lat.contattabMese, targets.contattab||60) : null, rule:"standard" },
                { label:"CPL", actual: fv2(lat.cplMese,"€"), tobe: (targets.cpl||12.5)+"€ target", delta: lat.cplMese ? calcPct(lat.cplMese, targets.cpl||12.5) : null, rule:"inverse" },
                { label:"CPA", actual: fv2(lat.cpaMese,"€"), tobe: (targets.cpa||100)+"€ target", delta: lat.cpaMese ? calcPct(lat.cpaMese, targets.cpa||100) : null, rule:"inverse" },
              ];

              return (
                <div style={{ background:CARD2, border:"2px solid "+P, borderRadius:"12px", overflow:"hidden", marginBottom:"20px" }}>
                  <div style={{ background:"linear-gradient(135deg,"+P+",#4a35a0)", padding:"14px 24px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <span style={{ color:"#fff", fontWeight:"bold", fontSize:"13px", fontFamily:FF }}>KPI Settimana · valori *Mese da {lat.date}</span>
                    <span style={{ color:"#EDE8FA", fontSize:"11px", fontFamily:FF }}>{filtered.length} giorni selezionati</span>
                  </div>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", background:P+"33", padding:"10px 20px", borderBottom:"2px solid "+P+"44" }}>
                    {["KPI","VALORE *MESE","TARGET / RANGE","DELTA"].map((h,i)=>(<span key={h} style={{ color:"#c4b8ff", fontSize:"7px", fontFamily:FF, fontWeight:"bold", letterSpacing:"2px", textAlign:i>0?"center":"left" }}>{h}</span>))}
                  </div>
                  {kpiRowsSett.map((row, i) => {
                    let deltaEl = <span style={{color:"#555",fontSize:"11px",fontFamily:FF}}>—</span>;
                    if (row.rule==="ctc") {
                      const cv=row.ctcVal; const cMin=row.ctcMin||25; const cMax=row.ctcMax||100; const ok=!isNaN(cv)&&cv>=cMin&&(cMax>=100?true:cv<=cMax); const c=ok?"#4caf50":"#ff5c5c"; const bg=ok?"#1a7a4018":"#c0392b18";
                      ...
                      if (row.rule==="standard") { if(d<=-5){c="#ff5c5c";bg="#c0392b18";icon="▼ ";}else if(d<0){c="#f5a623";bg="#f5a62318";icon="▼ ";}else if(d<=5){c="#4caf50";bg="#1a7a4018";icon="▲ ";}else{c="#1db954";bg="#0d5c2818";icon="▲ ";} }
                    ...
                    return (
                      <div key={row.label} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", padding:"12px 20px", borderBottom:i<kpiRowsSett.length-1?"1px solid "+BD:"none", background:i%2===0?"transparent":"#ffffff04", alignItems:"center" }}>
                        <div style={{color:"#b8a8ff",fontSize:"12px",fontFamily:FF,fontWeight:"bold"}}>{row.label}</div>
                        <div style={{textAlign:"center",color:"#fff",fontSize:"14px",fontFamily:FF,fontWeight:"bold"}}>{row.actual}</div>
                        <div style={{textAlign:"center",color:MU,fontSize:"11px",fontFamily:FF}}>{row.tobe}</div>
                        <div style={{textAlign:"center"}}>{deltaEl}</div>
            ...
            {/* MACROFORNITORI dal record più recente del periodo */}
            {(() => {
              const sorted = [...filtered].sort((a,b) => sortKey(b.date).localeCompare(sortKey(a.date)));
              const lat = sorted[0];
              if (!lat || !lat.macrofonti || !lat.macrofonti.length) return (
                <div style={{ ...cardCss, borderColor: O+"66", borderWidth:"2px", marginBottom:"20px" }}>
                  <div style={{ color:O, fontWeight:"bold", fontSize:"13px", fontFamily:FF, marginBottom:"6px" }}>📊 Macrofornitori</div>
                  <div style={{ color:MU, fontSize:"12px", fontFamily:FF }}>Nessun dato macrofornitori per questo periodo — inserisci gli screen di Power BI nel record più recente.</div>
                </div>
              );
              const macroToBe = getMacroToBe(lat.date);
              const redColorMf = (name, v) => { const val=parseFloat((v||"").replace(",",".")); const n=(name||"").toUpperCase(); if(isNaN(val)) return MU; if(n.includes("GOOGLE")) { return val>22?"#1db954":val>19.5?"#4caf50":val>=15.5?"#f5a623":"#ff5c5c"; } if(n.includes("SOCIAL")||n.includes("TRABOCCO")) { return val>15.5?"#4caf50":val>=12.5?"#f5a623":"#ff5c5c"; } return "#c4b8ff"; };
              const cpaColorMf = (v) => { const n=parseFloat(v); return isNaN(n)?MU:n>110?"#ff5c5c":n>103?"#f5a623":"#4caf50"; };
              return (
                <div style={{ background:CARD2, border:"2px solid "+O, borderRadius:"12px", overflow:"hidden", marginBottom:"20px" }}>
                  <div style={{ background:"linear-gradient(135deg,#8B3A0A,#c45510)", padding:"14px 24px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <span style={{ color:"#fff", fontWeight:"bold", fontSize:"13px", fontFamily:FF }}>MACROFORNITORI — {lat.date}</span>
                    <span style={{ color:"#FFD0B5", fontSize:"10px", fontFamily:FF }}>Dati dal record più recente del periodo</span>
                  </div>
                  <div style={{ display:"grid", gridTemplateColumns:"2fr 70px 70px 70px 70px 90px", background:O+"33", padding:"9px 20px", borderBottom:"2px solid "+O+"44" }}>
                    {["FORNITORE","LEADS","%","RED","CPA","vs TO BE"].map((h,i)=>(<span key={h} style={{ color:"#FFD0B5", fontSize:"8px", fontFamily:FF, fontWeight:"bold", letterSpacing:"1.5px", textAlign:i>0?"center":"left" }}>{h}</span>))}
                  </div>
                  {lat.macrofonti.map((mf,i) => {
                    const tb = macroToBe ? macroToBe[mf.name] : null;
                    const dTxt = tb ? (() => { const d=((mf.leads-tb)/tb*100).toFixed(1); return (parseFloat(d)>=0?"+":"")+d+"%"; })() : "—";
                    const dColor = tb ? (((mf.leads-tb)/tb)>=0?"#4caf50":"#ff5c5c") : MU;
                    return (
                      <div key={mf.name} style={{ display:"grid", gridTemplateColumns:"2fr 70px 70px 70px 70px 90px", padding:"10px 20px", borderBottom:"1px solid "+BD, background:i%2===0?"transparent":"#ffffff04", alignItems:"center" }}>
                        <span style={{ fontSize:"11px", color:"#FFD0B5", fontFamily:FF, fontWeight:"600" }}>{mf.name}</span>
                        <span style={{ fontSize:"12px", color:TX, textAlign:"center", fontFamily:FF }}>{mf.leads}</span>
                        <span style={{ fontSize:"11px", color:MU, textAlign:"center", fontFamily:FF }}>{mf.pct}%</span>
                        <span style={{ fontSize:"12px", fontWeight:"bold", color:redColorMf(mf.name,mf.red), textAlign:"center", fontFamily:FF }}>{mf.red !== "—" ? mf.red+"%" : "—"}</span>
                        <span style={{ fontSize:"12px", fontWeight:"bold", color:cpaColorMf(mf.cpa), textAlign:"center", fontFamily:FF }}>{mf.cpa !== "—" ? mf.cpa+"€" : "—"}</span>
                        <span style={{ fontSize:"11px", fontWeight:"bold", color:dColor, textAlign:"center", fontFamily:FF }}>{dTxt}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{ borderRadius: "10px", overflowX: "auto", border: "1px solid " + BD }}>
              <div style={{ display: "grid", gridTemplateColumns: mainTpl, gap: "6px", padding: "12px 16px", background: "linear-gradient(135deg," + P + ",#4a35a0)", fontSize: "11px", fontFamily: FF, fontWeight: "bold", color: "#fff", alignItems: "center", minWidth: "1100px" }}>
                {mainCols.map(c => <span key={c}>{c}</span>)}
              </div>
              {filtered.map((r, i) => (
                <div key={r.id} style={{ display: "grid", gridTemplateColumns: mainTpl, gap: "6px", padding: "11px 16px", background: i % 2 === 0 ? CARD : "#150e38", borderBottom: "1px solid " + BD, alignItems: "center", fontSize: "13px", fontFamily: FF, minWidth: "1100px" }}>
                  <span style={{ color: O, fontWeight: "bold", fontSize: "14px" }}>{r.date}</span>
                  <span style={{ color: TX }}>{r.leadGen || "—"}</span>
                  <span style={{ color: MU }}>{r.leadToBe || "—"}</span>
                  <Chip val={calcPct(r.leadGen, r.leadToBe)} />
                  <TimingBadge val={r.timing ? parseInt(r.timing) : null} />
                  <span style={{ color: TX }}>{r.ore || "—"}</span>
                  <span style={{ color: MU }}>{r.oreDich || "—"}</span>
                  <Chip val={calcPct(r.ore, r.oreDich)} />
                  <span style={{ color: TX }}>{r.lavOra || "—"}</span>
                  <span style={{ color: TX }}>{r.pressione || "—"}</span>
                  <span style={{ color: TX }}>{r.ctc ? r.ctc + "%" : "—"}</span>
                  <span style={{ color: TX }}>{r.cpl ? r.cpl + "€" : "—"}</span>
                  <span style={{ color: TX }}>{r.cpa ? r.cpa + "€" : "—"}</span>
                  <span style={{ color: TX }}>{r.red ? r.red + "%" : "—"}</span>
                  <span style={{ color: TX }}>{r.resa || "—"}</span>
                  <div style={{ display: "flex", gap: "5px" }}>
                    <button onClick={() => { setActiveReport(r); setView("report"); }} style={{ background: P + "33", border: "none", color: "#c4b8ff", borderRadius: "5px", padding: "5px 9px", cursor: "pointer", fontFamily: FF, fontSize: "13px" }}>📋</button>
                    <button onClick={() => { setForm({ ...r }); setView("form"); }} style={{ background: O + "33", border: "none", color: O, borderRadius: "5px", padding: "5px 9px", cursor: "pointer", fontFamily: FF, fontSize: "13px" }}>✏️</button>
                    <button onClick={async () => { const upd = records.filter(x => x.id !== r.id); setRecords(upd); await persist(upd); }} style={{ background: "#c0392b22", border: "none", color: "#ff5c5c", borderRadius: "5px", padding: "5px 9px", cursor: "pointer", fontFamily: FF, fontSize: "13px" }}>🗑</button>
                  </div>
                </div>
              ))}
              {periodAvg && (
                <div style={{ display: "grid", gridTemplateColumns: mainTpl, gap: "6px", padding: "12px 16px", background: "linear-gradient(90deg," + O + "25," + P + "20)", borderTop: "2px solid " + O, alignItems: "center", fontSize: "13px", fontFamily: FF, fontWeight: "bold", minWidth: "1100px" }}>
                  <span style={{ color: O, fontSize: "11px", letterSpacing: "1px" }}>{multi ? "MEDIA" : "TOT"}</span>
                  <span style={{ color: "#fff" }}>{periodAvg.leads}</span>
                  <span style={{ color: MU, fontWeight: "normal", fontSize: "12px" }}>—</span>
                  <Chip val={periodAvg.deltaLead} />
                  <TimingBadge val={periodAvg.timing ? Math.round(parseFloat(periodAvg.timing)) : null} />
                  <span style={{ color: "#fff" }}>{periodAvg.ore}</span>
                  <span style={{ color: MU, fontWeight: "normal", fontSize: "12px" }}>—</span>
                  <Chip val={periodAvg.deltaOre} />
                  <span style={{ color: "#fff" }}>{periodAvg.lavOra || "—"}</span>
                  <span style={{ color: "#fff" }}>{periodAvg.pressione || "—"}</span>
                  <span style={{ color: "#fff" }}>{avgOf(filtered.map(r=>r.ctc)) ? avgOf(filtered.map(r=>r.ctc)) + "%" : "—"}</span>
                  <span style={{ color: "#fff" }}>{periodAvg.cpl ? periodAvg.cpl + "€" : "—"}</span>
                  <span style={{ color: "#fff" }}>{periodAvg.cpa ? periodAvg.cpa + "€" : "—"}</span>
                  <span style={{ color: "#fff" }}>{periodAvg.red ? periodAvg.red + "%" : "—"}</span>
                  <span style={{ color: "#fff" }}>{periodAvg.resa || "—"}</span>
                  <span style={{ color: MU, fontSize: "11px" }}>{filtered.length} gg.</span>
                </div>
              )}
            </div>

            <div style={{ ...cardCss, marginTop: "22px" }}>
              <div style={{ ...secCss, color: P }}>Trend periodo</div>
              <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                {[...filtered].reverse().map(r => (
                  <div key={r.id} onClick={() => { setActiveReport(r); setView("report"); }}
                    style={{ background: CARD2, border: "1px solid " + BD, borderRadius: "10px", padding: "14px 18px", minWidth: "120px", cursor: "pointer" }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = P; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = BD; }}>
                    <div style={{ fontSize: "15px", color: O, fontWeight: "bold", marginBottom: "10px", fontFamily: FF }}>{r.date}</div>
                    {[["Lead",r.leadGen],["Timing",r.timing?r.timing+"%":"—"],["CTC",r.ctc?r.ctc+"%":"—"],["CPL",r.cpl?r.cpl+"€":"—"],["CPA",r.cpa?r.cpa+"€":"—"],["RED",r.red?r.red+"%":"—"],["Resa",r.resa||"—"]].map(([l,v])=>(
                      <div key={l} style={{ fontSize: "13px", color: MU, fontFamily: FF, marginBottom: "4px" }}>{l}: <span style={{ color: TX, fontWeight: "bold" }}>{v}</span></div>
                    ))}
                  </div>
                ))}
              </div>
        ...
        {filtered.length > 0 && <WeeklySintesiBox filtered={filtered} records={records} />}
        {filtered.length > 0 && <RecapBox buildFn={buildStoricRecap} recs={[...filtered].reverse()} title="Recap KPI — testo" />}
      </div>
    );
  };
  ...
  const renderMese = () => {
    ...
    const kpiRows = [
      ...
      { label:"ORE / GG", actual: dailyAvgI("ore") ? dailyAvgI("ore").toLocaleString("it-IT") : "—", tobe: latestRec.oreDich ? latestRec.oreDich+" dich." : "—", delta: dailyAvgI("ore") && latestRec.oreDich ? calcPct(dailyAvgI("ore"), toNum(latestRec.oreDich)) : null, rule:"standard" },
