...
function WeeklyLoadForm({ weekOptions, defaultWeekId, tobeSabati, recordsSett, records, onClose, onSaved }) {
  ...
  // Upload separati: settimana lun-ven (KPI principali) e sabato (giorno separato)
  const [screensSett, setScreensSett] = useState([]);
  const [screensSat, setScreensSat] = useState([]);
  const [extracting, setExtracting] = useState(false);
  ...
  const handleFilesUpload = (files, which) => {
    const newScreens = [];
    let processed = 0;
    [...files].forEach((file, idx) => {
      ...
      reader.onload = (e) => {
        ...
        if (processed === files.length) {
          if (which === "sett") setScreensSett(prev => [...prev, ...newScreens]);
          else if (which === "sat") setScreensSat(prev => [...prev, ...newScreens]);
        }
      };
      reader.readAsDataURL(file);
    });
  ...
  const handleExtract = async () => {
    if (screensSett.length === 0) {
      setExtractMsg("⚠️ Carica almeno uno screenshot SETTIMANALE (lun-ven) prima di estrarre.");
      return;
    }
    ...
    setExtracting(true);
    setExtractMsg("Estrazione AI in corso con Opus (solo screen settimana lun-ven)...");

    // CRITICO: solo screen settimanali vanno a Opus.
    // Gli screen sabato sono mostrati separatamente all'utente (ma NON entrano nell'estrazione settimanale,
    // per evitare che Opus confonda Economics sabato con Economics settimana lun-ven).
    const imgContent = screensSett.map(s => ({
      type: "image",
  ...
  return (
    <div style={{
      ...
      {/* Box upload 1: SCREEN SETTIMANALI lun-ven (per KPI principali del Report Settimana) */}
      <div style={{ marginBottom: "12px" }}>
        <div style={{ fontSize: "11px", color: "#c4b8e3", fontWeight: "bold", marginBottom: "6px", letterSpacing: "0.5px" }}>
          📊 1. SCREEN SETTIMANA LUN-VEN <span style={{ color: "#ff8a4c", fontWeight: "normal" }}>(obbligatori per estrazione KPI)</span>
        </div>
        <input
          type="file"
          multiple
          accept="image/*"
          onChange={e => handleFilesUpload(e.target.files, "sett")}
          style={{ display: "none" }}
          id="weekly-upload-input-sett"
        />
        <label htmlFor="weekly-upload-input-sett" style={{
          display: "block",
          padding: "20px",
          textAlign: "center",
          background: "#1a1232",
          ...
          📷 Clicca per caricare gli screen del range <strong style={{ color: "#c4b8e3" }}>lun-ven</strong> (Economics, Contattab, Timing, Resa, Lavora, Ore, ecc.)
        </label>
        {screensSett.length > 0 && (
          <div style={{ marginTop: "6px", fontSize: "11px", color: "#6bcf5e" }}>
            ✓ {screensSett.length} screen settimanali caricati: {screensSett.map(s => s.name).join(", ")}
          </div>
        )}
      </div>

      {/* Box upload 2: SCREEN SABATO (informativi, NON usati per estrazione KPI lun-ven) */}
      <div style={{ marginBottom: "12px" }}>
        <div style={{ fontSize: "11px", color: "#c4b8e3", fontWeight: "bold", marginBottom: "6px", letterSpacing: "0.5px" }}>
          📅 2. SCREEN SABATO <span style={{ color: "#8a7ca8", fontWeight: "normal" }}>(opzionale, NON entrano nell'estrazione KPI lun-ven)</span>
        </div>
        <input
          type="file"
          multiple
          accept="image/*"
          onChange={e => handleFilesUpload(e.target.files, "sat")}
          style={{ display: "none" }}
          id="weekly-upload-input-sat"
        />
        <label htmlFor="weekly-upload-input-sat" style={{
          display: "block",
          padding: "16px",
          textAlign: "center",
          background: "#0f0c1e",
          border: "1px dashed " + BD + "66",
          borderRadius: "6px",
          color: "#8a7ca8",
          cursor: "pointer",
          fontSize: "11px"
        }}>
          📷 Screen del sabato (per il record giornaliero del sabato, da inserire da "+ Inserisci")
        </label>
        {screensSat.length > 0 && (
          <div style={{ marginTop: "6px", fontSize: "10px", color: "#8a7ca8", fontStyle: "italic" }}>
            {screensSat.length} screen sabato caricati (non passati a Opus per il Report Settimana): {screensSat.map(s => s.name).join(", ")}
          </div>
        )}
      </div>
