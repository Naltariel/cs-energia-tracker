...
function WeeklyView({ record, tobeSabati, records, activeWeek, recordsSett, allWeeks, onRecordUpdate }) {
  ...
  return (
    <>
      ...
      {/* ── TIMING FASCE FUORI RANGE (media settimana) ── */}
      <div style={cardCss}>
        <div style={{ ...secCss, color: O, marginBottom: "12px" }}>
          — Timing 1-5 min — Fasce fuori range (media settimana) —
        </div>
        {tuteFasceFuoriRange.length > 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "2px", marginBottom: "12px" }}>
              {tuteFasceFuoriRange.map(f => {
                ...
                const leadsStr = f.leadsEff !== null && f.tobe !== null
                  ? " · " + Math.round(f.leadsEff) + " lead/gg vs " + Math.round(f.tobe) + " TO BE"
                  : "";
                return (
                  <div key={f.h} style={{
                    display: "flex",
                    ...
                    <span style={{ fontSize: "15px" }}>{dot}</span>
                    <span style={{ color: "#fff", fontWeight: 500, minWidth: "55px", fontSize: "13px" }}>{f.h}:00</span>
                    <span style={{ color: "#fff", fontWeight: 600, minWidth: "50px", fontSize: "13px" }}>{f.timing}%</span>
                    <span style={{ color: "#8a7ca8", fontSize: "11px" }}>{leadsStr}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ padding: "14px", textAlign: "center", color: "#6bcf5e", fontSize: "13px" }}>
      ...
      <div style={cardCss}>
        <div style={{ ...secCss, color: P, marginBottom: "12px" }}>
          Pattern mese (da ultimo record giornaliero disponibile{latestRecWithMese ? ": " + latestRecWithMese.date : ""})
        </div>
        {hasPatternMese ? (
          <div style={{ fontSize: "13px", color: "#c4b8e3", lineHeight: "1.8" }}>
            {patternMeseOver.length > 0 && (
              <>
                <strong style={{ color: "#ff6b6b" }}>Over-generazione cronica (ridurre):</strong> fasce {patternMeseOver.map(f => f.h + ":00 (" + f.v + "%)").join(", ")}
                {patternMeseUnder.length > 0 && <br/>}
              </>
            )}
            {patternMeseUnder.length > 0 && (
              <>
                <strong style={{ color: "#FD6F3B" }}>Under-generazione cronica (aumentare):</strong> fasce {patternMeseUnder.map(f => f.h + ":00 (" + f.v + "%)").join(", ")}
              </>
            )}
          </div>
        ) : hourlyMese ? (
          <div style={{ padding: "14px", textAlign: "center", color: "#6bcf5e", fontSize: "13px" }}>
            ✓ Nessuna fascia strutturalmente fuori range nel mese
          </div>
        ) : (
          <div style={{ padding: "14px", textAlign: "center", color: "#8a7ca8", fontSize: "13px" }}>
            Pattern mese non disponibile (nessun record giornaliero con hourlyMese)
          </div>
        )}
      </div>
