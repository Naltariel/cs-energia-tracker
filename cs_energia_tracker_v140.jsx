...
function detectScreenCategory(filename) {
  ...
  // Ordine: prima i "mese" perché contengono il pattern del giornaliero + "1"
  if (n.includes("economics1") || (n.includes("economics") && /economics\d?1\d/.test(n))) return "economics_mese";
  if (n.startsWith("economics") || n.includes("economics")) return "economics_gg";
  if (n.startsWith("contattab1") || /contattab\d?1\d/.test(n)) return "contattab_mese";
  if (n.startsWith("contattab")) return "contattab_gg";
  if (n.startsWith("timing1") || /timing\d?1\d/.test(n)) return "timing_mese";
...
Rispondi SOLO con il JSON o "null", senza commenti, senza markdown.`;
      ...
      const [focusResa, focusOre, focusLavOra, focusTiming, focusRed, focusRedMese, focusResaMese, focusContattabMese, focusContattab, focusPressione, focusHourly, focusHourlyMese, ...cpaResults] = await Promise.all([
        extractWithFocus(imgContent.slice(0, -1), prompts.resa, "resa"),
        extractWithFocus(imgContent.slice(0, -1), prompts.ore, "ore"),
        extractWithFocus(imgContent.slice(0, -1), prompts.lavOra, "lavOra"),
        extractWithFocus(imgContent.slice(0, -1), prompts.timing, "timing"),
        extractWithFocus(imgContent.slice(0, -1), prompts.red, "red"),
        extractWithFocus(imgContent.slice(0, -1), prompts.redMese, "redMese"),
        extractWithFocus(imgContent.slice(0, -1), prompts.resaMese, "resaMese"),
        extractWithFocus(imgContent.slice(0, -1), prompts.contattabMese, "contattabMese"),
        extractWithFocus(imgContent.slice(0, -1), prompts.contattab, "contattab"),
        extractWithFocus(imgContent.slice(0, -1), prompts.pressione, "pressione"),
        extractHourly(),
        extractHourlyMese(),
        ...cpaPromises
      ]);
