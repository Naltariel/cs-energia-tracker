...
function WeeklyRecapBtn({ record, activeWeek, tobeSabati, records }) {
  ...
  const generate = () => {
    ...
    macrofonti.forEach(m => {
      ...
      const nonScalabile = isETC || (isTrab && pctVal <= 2);
...
function SintesiBox({ r, records, recordsSett, onSave }) {
  ...
  const buildCtx = () => {
    ...
    fornitoriAnalisi.forEach(({ nameReale, nameCanonico, tipo }) => {
      ...
      } else if (tipo === "TRABOCCO" && pctLead <= 2) {
        azioneSym = "⚪"; azioneLabel = "Non scalabile";
        note = "Volume trabocco sotto soglia 2%, non scalabile";
...
async function parseExcelFile(file) {
  ...
  // Se il nome diceva "giornaliero" ma i filtri Date hanno un range > 2 giorni → period = cumulato_mese
  // (questo è il fallback robusto per quando l'utente non rinomina i file)
  if (parsed && parsed.filtri && parsed.filtri.startDate && parsed.filtri.endDate) {
    ...
    // diffDays = 1 → giornaliero (es. 18/05 → 19/05 indica filtro su 18/05)
    // diffDays = 7 → settimanale lun-ven + sabato/domenica
    // diffDays > 7 → cumulato mese
...
function aggregateExcelToRecord(parsedFiles, targetDate) {
  ...
  parsedFiles.forEach(({ type, period, parsed }) => {
    ...
    if (type === "economics" && (period === "giornaliero" || period === "settimanale")) {
      if (parsed.total) {
...
function WeeklyView({ record, tobeSabati, records, activeWeek, recordsSett, allWeeks, onRecordUpdate }) {
  ...
  macroRows.forEach(r => {
    ...
    // Trabocco con % ≤ 2% → non scalabile
    const name = (r.name || "").toUpperCase();
    const isETC = name === "ETC";
    const isTrabocco = name.includes("TRABOCCO");
    const pctSmall = r.pctLunVen != null && r.pctLunVen <= 2;
    r.nonScalabile = isETC || (isTrabocco && pctSmall);
