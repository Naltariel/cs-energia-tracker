...
function WeeklyView({ record, tobeSabati, records, activeWeek, recordsSett, allWeeks, onRecordUpdate }) {
  ...
  const generaConfronto = async () => {
    ...
    const prompt = "Sei un analista marketing operativo di ComparaSemplice.it. Confronta due settimane di performance del tracker CS Energia.\n\n" +
      ...
      "- Per grandezze assolute (lead, ore, €, durata in minuti): usa variazione percentuale, es. '-5,8%' o '+€4'\n" +
      "- Per KPI già in % (RED, CTC, timing, contattabilità, resa): usa punti percentuali, es. '-1,3pp'\n" +
      "- NON usare MAI 'basis points' o 'bp'\n\n" +
      "STRUTTURA (3 paragrafi, max 140 parole totali):\n\n" +
      "PARAGRAFO 1 — Volumi e produttività:\n" +
      "- Cita Lead/gg e Ore/gg con delta vs settimana precedente (NON vs TO BE in questo paragrafo)\n" +
      "- Cita LAV/ORA con delta\n" +
      "- Cita DURATA MEDIA con delta (dato: kpi.durataMedia, in minuti)\n" +
      "- Esempio: 'Lead/gg: 2.389 → 2.274 (-4,8%). Ore/gg: 970 → 956 (-1,4%). LAV/ORA 2,5 → 2,4 (-4,0%). Durata media 6,28 → 8,22 min (+30,9%).'\n\n" +
      "PARAGRAFO 2 — Economics:\n" +
      "- Cita CPA, CPL, RED con delta\n" +
      "- REGOLA CRITICA — 'tutti KPI migliorati':\n" +
      "  SE il CPA è DIMINUITO (delta negativo) E il CPL è DIMINUITO E la RED è AUMENTATA (delta positivo):\n" +
      "    DEVI aprire il paragrafo con: 'Miglioramento su tutti i KPI principali (CPA, CPL, RED).'\n" +
      "    Poi commentare la DURATA: se è stabile (variazione entro ±5%) di' 'Durata stabile.', se è migliorata di' 'Durata in miglioramento.', se è peggiorata (variata oltre ±5% in senso sfavorevole) di' 'Durata in peggioramento.'\n" +
      "- SE 2 KPI su 3 (CPA/CPL/RED) sono migliorati: cita il positivo e il negativo separatamente (no affermazioni generaliste)\n" +
      "- Identifica chi guida (il macrofornitore che più ha spostato il CPA totale)\n\n" +
      "PARAGRAFO 3 — Macrofornitori:\n" +
      "- Cita 2-3 fornitori più rilevanti per volume o variazione\n" +
      "- Per ciascuno: delta CPA e/o RED vs settimana precedente\n" +
      "- Se c'è una nota positiva chiudi con quella\n\n" +
      "REGOLE GENERALI:\n" +
  ...
  return (
    <>
      ...
      {/* ── SEZIONE CONFRONTO SETTIMANA PRECEDENTE ── */}
      {recordPrec && (
        <div style={cardCss}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
            <div style={{ ...secCss, color: P, marginBottom: 0 }}>
              — Confronto vs settimana {fmtDDMM(settPrec.mon)}-{fmtDDMM(settPrec.sat)} —
            </div>
            <button
          ...
          {errConfronto && (
            <div style={{ padding: "10px 12px", background: "rgba(255,107,107,0.08)", border: "1px solid rgba(255,107,107,0.25)", borderRadius: "4px", fontSize: "12px", color: "#ff6b6b", marginBottom: "12px" }}>
              Errore generazione confronto: {errConfronto}
            </div>
          )}
