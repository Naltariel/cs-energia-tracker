...
function WeeklyRecapBtn({ record, activeWeek, tobeSabati, records, macroRows, fasceAnalisi, sintesiFasce, patternMeseOver, patternMeseUnder, latestRecWithMese, avgLeadGg, avgTobeLeadGg, avgOreGg, avgTobeOreGg, satLeadEff, satOreEff, sabato, fmtDDMM }) {
  const [lines, setLines] = useState([]);
  const [copied, setCopied] = useState(false);
  ...
  const generate = () => {
    ...
    if (sabato || satLeadEff != null) {
      ...
      if (satLeadEff != null && sabato?.lead) L.push("- Lead: " + satLeadEff.toLocaleString() + " vs " + sabato.lead.toLocaleString() + " TO BE (" + fmtPct(pct(satLeadEff, sabato.lead)) + ")");
      else if (satLeadEff != null) L.push("- Lead: " + satLeadEff.toLocaleString());
      if (satOreEff != null && sabato?.ore) L.push("- Ore: " + satOreEff.toLocaleString() + " vs " + sabato.ore.toLocaleString() + " TO BE (" + fmtPct(pct(satOreEff, sabato.ore)) + ")");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "8px" }}>
      <div style={{ display: "flex", gap: "8px" }}>
        <button onClick={generate} style={{ background: "#FD6F3B", border: "none", borderRadius: "8px", padding: "9px 16px", fontSize: "13px", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontWeight: "bold", color: "#fff", cursor: "pointer" }}>📋 Genera Recap</button>
      ...
      {lines.length > 0 && (
        <pre style={{ background: "#0f0c1e", border: "1px solid #664CCD44", borderRadius: "10px", padding: "16px 18px", fontSize: "12px", fontFamily: "'IBM Plex Mono','Courier New',monospace", lineHeight: "1.6", whiteSpace: "pre-wrap", margin: "0", maxWidth: "100%", textAlign: "left", color: "#d8cfec" }}>
          {lines.join("\n")}
        </pre>
      )}
    </div>
                ...
                {macroRows.map((m, idx) => {
                  ...
                  return (
                    <tr key={idx} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                      ...
                      <td style={{ padding: "7px 6px", color: "#fff", fontSize: "11px", textAlign: "right" }}>{m.cpa ? "€" + m.cpa : "—"}</td>
                    </tr>
                  );
                })}
              </tbody>
